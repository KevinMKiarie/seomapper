...
import type { ScanResult, Issue, PageResult, Severity } from '../types.js';
...
function renderPage(page: PageResult, idx: number): string {
  ...
  const issuesHtml = page.issues.length
    ? page.issues.map(iss => renderIssue(iss)).join('')
