...
const TOOLS = [
  {
    name: 'scan_site',
    description:
      'Crawl a website and return SEO issues for all pages. Returns scores, issues by severity, and fix hints.',
    inputSchema: {
      type: 'object',
      properties: {
...
const SEO_RULES = [
  ...
  { id: 'html-lang-missing',           category: 'meta',       description: 'Missing lang attribute on <html> — required for accessibility tree and international SEO' },
  ...
  { id: 'og-type-missing',             category: 'og',         description: 'Missing og:type (website, article, product, etc.)' },
  { id: 'og-url-missing',              category: 'og',         description: 'Missing og:url — anchors social sharing to the canonical URL' },
  ...
  { id: 'generic-anchor-text',         category: 'content',    description: 'Links use "click here" or "read more" — no destination signal' },
  { id: 'image-filename-generic',      category: 'content',    description: 'Images have non-descriptive filenames (img001.jpg, etc.)' },
  { id: 'content-freshness-missing',   category: 'content',    description: 'Article page with no date signal — Google cannot evaluate freshness' },
  ...
  // Links
  { id: 'nofollow-internal',           category: 'links',      description: 'Internal links carry rel="nofollow" — blocks internal PageRank flow' },
  ...
  { id: 'organization-schema-missing', category: 'aeo',        description: 'No Organization schema — weakens site identity signals' },
  { id: 'speakable-missing',           category: 'aeo',        description: 'Article page with no Speakable schema — AI voice assistants cannot identify key sections' },
  { id: 'video-schema-missing',        category: 'aeo',        description: 'Video content without VideoObject schema' },
  { id: 'product-schema-missing',      category: 'aeo',        description: 'Product page without Product schema — misses price/availability rich results' },

  // Security
  { id: 'http-not-https',              category: 'security',   description: 'Page served over HTTP — Google penalises non-HTTPS' },
  { id: 'mixed-content',               category: 'security',   description: 'HTTP resources on an HTTPS page — browsers block these' },
...
export async function startMcpServer(): Promise<void> {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === 'list_rules') {
      return {
        content: [{ type: 'text' as const, text: JSON.stringify(SEO_RULES, null, 2) }],
      };
    }
    ...
    if (name === 'scan_page') {
      ...
      return {
        content: [{ type: 'text' as const, text: JSON.stringify(result.pages[0] ?? {}, null, 2) }],
      };
    }
