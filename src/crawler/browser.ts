...
// Playwright is an optional peer dependency — install it separately if you scan JS-heavy sites:
...
export async function fetchWithBrowser(url: string): Promise<ParsedPage> {
  ...
  try {
    ...
    throw new Error(
      'Playwright is not installed. To scan JavaScript-rendered pages run:\n' +
      '  npm install playwright && npx playwright install chromium'
    );
  }
